...
Mat box_filter_modified_tipri(const Mat& I, int h, int v) {
	Mat result;
...
Mat guided_filter_tipri(const Mat& originG, const Mat& originR, const Mat& mask, const Mat& originI, const Mat& originP, const Mat& M, int h = 2, int v = 2, float eps = 0.0) {
	...
	Mat N = box_filter_modified_tipri(M, h, v);
	Mat temp = (N == 0);
	temp.convertTo(temp, CV_32F, 1.0 / 255.0);
	N = N + temp;
	// The size of each local patch; N=(2h+1)*(2v+1) except for boundary pixels.
	Mat N2 = box_filter_modified_tipri(Mat::ones(I.rows, I.cols, CV_32F), h, v);

	Mat mean_IP = box_filter_modified_tipri(I.mul(p).mul(M), h, v);
	divide(mean_IP, N, mean_IP);
	Mat mean_II = box_filter_modified_tipri(I.mul(I).mul(M), h, v);
	...
	Mat N3 = box_filter_modified_tipri(mask, h, v);
	Mat temp1 = (N3 == 0);
	temp1.convertTo(temp1, CV_32F, 1.0 / 255.0);
	N3 = N3 + temp1;
	Mat mean_G = box_filter_modified_tipri(G.mul(mask), h, v);
	divide(mean_G, N3, mean_G);
	Mat mean_R = box_filter_modified_tipri(R.mul(mask), h, v);
	...
	divide(1.0, dif, dif);
	Mat wdif = box_filter_modified_tipri(dif, h, v);
	...
	//Mean a and Mean b
	Mat mean_a = box_filter_modified_tipri(a.mul(dif), h, v);
	divide(mean_a,wdif, mean_a);
	Mat mean_b = box_filter_modified_tipri(b.mul(dif), h, v);
