...
using namespace std;
using namespace cv;
void toSingleChanneltipri(cv::Mat& src, cv::Mat& dst) {
	if (src.channels() != 3) {
...
// 0 R G
// 1 G B
void bayer_splittipri(cv::Mat& Bayer, cv::Mat& Dst) {
	if (Bayer.channels() != 1) {
...
void bayer_masktipri(cv::Mat& Bayer, cv::Mat& Dst) {
	Mat temp = cv::Mat::ones(Bayer.size(), CV_8U);
	bayer_splittipri(temp, Dst);// or split(Bayer, temp)
}

void demosaic_tipri(cv::Mat& Bayer, cv::Mat& Dst, float sigma = 1.0) {
	cv::Mat Src = Bayer.clone();
	if (Bayer.channels() == 1) { //input 1 channel -> 3 channel Bayer
		bayer_splittipri(Bayer, Src); // or split(Bayer, Src);
	}

	Mat Src1ch;
	toSingleChanneltipri(Src, Src1ch); // or merge(Src, Src1ch);
	Src1ch.convertTo(Src1ch, CV_32F, 1.0 / 255.0); //normalize

	Mat tempMask;
	bayer_masktipri(Src, tempMask);
	...
	Mat tentativeR_H = guided_filter_tipri(GuideG_H, bgr[2], mask[2], diffGr1H, diffr1H, mask[2], h, v, eps);//G,R,mask,I,P,M,h,v,eps
	...
	Mat tentativeB_H = guided_filter_tipri(GuideG_H, bgr[0], mask[0], diffGb1H, diffb1H, mask[0], h, v, eps);
	Mat diffb2H, diffGb2H;
	filter2D(bgr[1].mul(maskGb), diffGb2H, -1, FHK);
	filter2D(GuideB_H.mul(maskGb), diffb2H, -1, FHK);
	Mat tentativeGb_H = guided_filter_tipri(GuideB_H, bgr[1].mul(maskGb), maskGb, diffb2H, diffGb2H, maskGb, h, v, eps);
	...
	Mat tentativeR = guided_filter_tipri(finalBGR[1], bgr[2], mask[2], lap_green1, lap_red, mask[2], h, v, eps);//G,R,mask,I,P,h,v,eps
	...
	Mat tentativeB = guided_filter_tipri(finalBGR[1], bgr[0], mask[0], lap_green2, lap_blue, mask[0], h, v, eps);
