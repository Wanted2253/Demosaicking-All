...
using namespace std;
using namespace cv;
void toSingleChannelri(cv::Mat& src, cv::Mat& dst) {
	if (src.channels() != 3) {
...
// 0 R G
// 1 G B
void bayer_splitri(cv::Mat& Bayer, cv::Mat& Dst) {
	if (Bayer.channels() != 1) {
...
void bayer_maskri(cv::Mat& Bayer, cv::Mat& Dst) {
	Mat temp = cv::Mat::ones(Bayer.size(), CV_8U);
	bayer_splitri(temp, Dst);// or split(Bayer, temp)
}
...
void demosaic_residual(cv::Mat& Bayer, cv::Mat& Dst, float sigma = 1.0) {
	...
	if (Bayer.channels() == 1) { //input 1 channel -> 3 channel Bayer
		bayer_splitri(Bayer, Src); // or split(Bayer, Src);
	}

	Mat Src1ch;
	toSingleChannelri(Src, Src1ch); // or merge(Src, Src1ch);
	Src1ch.convertTo(Src1ch, CV_32F, 1.0 / 255.0); //normalize

	Mat tempMask;
	bayer_maskri(Src, tempMask);
